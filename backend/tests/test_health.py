...
def test_chat_endpoint_runs_end_to_end(monkeypatch):
    # classifier.classify() (#3), planner.plan() (#4), and now every
    # specialist (#6-#9) all call the real LLM — mock all of them so this
    # test stays network-free and runs in CI without an API key.
    # Verification (still a stub, auto-approves) is the only remaining
    # unmocked agent. Any future agent that starts calling the real LLM
